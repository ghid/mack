...
class Mack {
		...
		dv :=  {  A						: 0
				...
				, types					: { "autohotkey": "*.ahk"
										  , "batch"     : "*.bat *.cmd"
                    	 			      ...
                    	 			      , "yaml"      : "*.yaml *.yml" }
				, types_expr			: Mack.regex_of_types()
				, type					: []
				, type_ignore			: []
				, v				  		: false
				, version		  		: false
	...
	refine_file_pattern(ByRef file_pattern) {
		...
		; Append '*.*' if the given name is a directory and no pattern is specified
		file_attrs := FileExist(file_pattern)
		if (InStr(file_attrs, "D")) {
			SplitPath file_pattern, name_pattern
			if (name_pattern = "" || !RegExMatch(name_pattern, "\*\.?\*?$")) {
	...
	/*
	 * Method:	add_to_list
	 *			Add a file pattern to a list of Mack.Option.
	 *
	 * Parameter:
	 *			list_name - name ot the list of Mack.Option
	 *			pattern - the pattern to add to the list
	 *
	 * Remarks:
	 *			The file pattern will be converted to a regular expression
	 *			before it is added to the list.
	 */
	add_to_list(list_name, pattern) {
		_log := new Logger("class." A_ThisFunc)

		if (_log.Logs(Logger.Input)) {
			_log.Input("list_name", list_name)
			_log.Input("pattern", pattern)
		}

		pattern := Mack.regex_of_file_pattern(pattern)
		if (_log.Logs(Logger.Finest)) {
			_log.Finest("pattern", pattern)
		}

		for _i, entry in Mack.Option[list_name] {
			if (entry = pattern) {
				return _log.Exit()
			}
		}
	 ...
	 *			Remove a file pattern from a list of Mack.Option.
	 *
	 * Parameter:
	 *			list_name - Name of the list of Mack.Option
	 *			name - the pattern to remove from the list
	 *
	 * Remarks:
	 *			The file pattern will be converted to a regular expression
	 *			before it is added to the list.
	...
	remove_from_list(list_name, name) {
		...
		pattern := Mack.regex_of_file_pattern(name)
		if (_log.Logs(Logger.Finest)) {
			_log.Finest("pattern", pattern)
		}
		...
		for _i, entry in Mack.Option[list_name] {
			if (entry = pattern) {
				Mack.Option[list_name].Remove(_i)
				break
			}
		}
	...
	/*
	 * Method: regex_of_types
	 *
	 * Returns: All registered filetypes as an regular expression.
	 */
	regex_of_types() {
		_log := new Logger("class." A_ThisFunc)

		expr := "("
		for _type, _file_pattern in Mack.Option.types {
	...
	/*
	 * Method:	regex_of_file_pattern
	 *			Convert a common file pattern to a regular expression.
	 * 
	 * Parameter:
	 *			file_pattern - A file pattern
	 *
	 * Returns:
	 *			A regex representation of the given file pattern
	 *
	 * Example:
	 *			>Mack.regex_of_file_pattern("*.*")	; Returns ".*?\..*?"
	 *			>Mack.regex_of_file_pattern(".git")	; Returns "\.git"
	 */
	 ...
	 *
	 * Returns:
	 *			A regular expression containing the content of the list.
	 */
	...
	process_line(line) {
		...
		return _log.Exit(Pager.Write(Ansi.Readable(line, Mack.Option.color), false))
	}
	 ...
	 *
	 * Returns:
	 *			A string containg the/all values from 'a'.
	 */
	array_to_string(a) {
		if (a.MaxIndex()) {
	...
	cli() {
		...
		op.Add(new OptParser.Callback(0, "ignore-dir", Mack.Option, "ignore_dir", "ignore_dir", "name", "Add/remove directory from list of ignored dirs", OptParser.OPT_ARG | OptParser.OPT_NEG))
		...
		op.Add(new OptParser.Callback(0, "type-del", Mack.Option, "type_del", "del_type", "X", "Remove all filters associated with X", OptParser.OPT_ARG))
	...
	run(args) {
		...
		try {
			...
			if (Mack.Option.sel_types) {
				for i, filetype in Mack.Option.sel_types {
					if (SubStr(filetype, 1, 1) = "!") {
						type_filter(SubStr(filetype, 2), true)
					} else {
						type_filter(filetype)
					}
				}
			...
			Mack.Option.match_ignore_dirs := Mack.regex_match_list(Mack.Option.ignore_dirs)
			...
			Pager.bEnablePager := Mack.Option.pager
...
/*
 * Function: ignore_dir
 *			Callback functionn for option parser to add or remove a directory pattern to or from ignore-list.
 *
 * Parameter:
 *			name - Directory pattern
 *			no_opt - if not set, add directory to ignore-list, otherwise remove it.
 */
ignore_dir(name, no_opt = "") {
	_log := new Logger("app.mack." A_ThisFunc)
	...
	if (no_opt = "") {
		Mack.add_to_list("ignore_dirs", name)
	} else {
		Mack.remove_from_list("ignore_dirs", name)
	}
 ...
 *			Callback functionn for option parser to add or remove a file pattern to or from ignore-list.
 *
 * Parameter:
 *			name - file pattern
 *			no_opt - if not set, add file pattern to ignore-list, otherwise remove it.
 */
ignore_file(name, no_opt = "") {
	_log := new Logger("app.mack." A_ThisFunc)
	...
	if (no_opt = "") {
		Mack.add_to_list("ignore_files", name)
	} else {
		Mack.remove_from_list("ignore_files", name)
	}
...
/*
 * Function: del_type
 *			Callback function for option parser to delete a filetype from Mack.Option.types.
 *
 * Parameter:
 *			filetype - The filetype to remove
 *
 * Throws:
 *			An exception if the given filetyp is unknown.
 *
 */
del_type(filetype) {
	_log := new Logger("app.mack." A_ThisFunc)
	...
	if (Mack.Option.types.Remove(filetype) = "") {
		throw _log.Exit(Exception("Unkwonw filetype: " filetype))
	}

	Mack.Option.regex_of_types := Mack.regex_of_types()
...
/*
 * Function: add_type
 *			Callback function for option parser to add a file pattern to an already defined filetype.
 * 
 * Parameter:
 *			filetype_filter - "<filetype-name>:<file-pattern>[+<file-pattern]..."
 *
 * Throws:
 *			An exception if the filetype filter is not vaild.
 *			An exception if the filetype is not defined.
 *
 * Remarks:
 *			filetype-filter will be added to Mack.option.types list.
 */
add_type(filetype_filter) {
	_log := new Logger("app.mack." A_ThisFunc)
	...
	Mack.Option.types[$1] .= " " filter

	if (_log.Logs(Logger.All)) {
		_log.All("Mack.Option.types:`n" LoggingHelper.Dump(Mack.Option.types))
	}
...
/*
 * Function: set_type
 *			Callback funtion for option parser to set a new filetype.
 *
 * Parameter:
 *			filetype_filter - "<filetype-name>:<file-pattern>[+<file-pattern]..."
 *
 * Throws:
 *			An exception if the filetype filter is not vaild.
 *			An exception if the filetype is already defined.
 *
 * Remarks:
 *			filetype-filter will be added to Mack.option.types list.
 *
 ...
 *			Callback function for option parser to include/exculde filetypes.
 *
 * Parameter:
 *			filetype - The filetype to include/exclude
 *			no_opt - if empty, filetype will be included, otherwise it will be excluded.
 *
 * Throws:
 *			An exception if the given filetype is unknown.
 *
 * Remarks:
 *			The given filttype will be translated to it's according pattern(s).
 *			Those pattern(s) will be stored in Mack.Option.type for included filetypes
 *			and in Mack.Option.type_ignore for excluded filetypes.
 */
type_filter(filetype, no_opt = "") {
	_log := new Logger("app.mack." A_ThisFunc)
	...
	if (no_opt = "") {
		Mack.add_to_list("type", Mack.Option.types[filetype])
		if (_log.Logs(Logger.All)) {
			_log.All("Mack.Option.type:`n" LoggingHelper.Dump(Mack.Option.type))
		}
	} else {
		Mack.add_to_list("type_ignore", Mack.Option.types[filetype])
		if (_log.Logs(Logger.All)) {
			_log.All("Mack.Option.type_ignore:`n" LoggingHelper.Dump(Mack.Option.type_ignore))
		}
	}
